var pages = 6;
var currentpage = 1;
for portfolio_item in site.portfolio {
  currentpage = portfolio_item.number;
}
...
function scrolltocurrent() {
	...
	var pageheight = currentdiv.position().top;
	...
	if (animatingdown == false) {
		if ($(window).scrollTop()<=$("#page"+(currentpage)).offset().top-50) {
			if (prevpage < currentpage) {
				var p2 = $( "#page"+(currentpage) );
				var pageheight = currentdiv.position().top-$(window).height();
				animatingup = true;
